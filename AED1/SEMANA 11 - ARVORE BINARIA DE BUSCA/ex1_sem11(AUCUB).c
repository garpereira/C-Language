...
int verifABBE(PArv ABB, int NORAIZ){
    if(ABB != NULL){
        ...
        if(ABB->chave == NORAIZ){//verifico a partir das subarvores se os nos sao validos
            verifABBE(ABB->esq, ABB->esq->chave);
            verifABBE(ABB->dir, ABB->dir->chave);
        }
    }
    return 0;
}

void imprime2(PArv ABB){
    if(ABB != NULL){
        printf("(%d", ABB->chave);
        imprime2(ABB->esq);
        imprime2(ABB->dir);
...
void uniao(PArv ABBA,PArv ABBC){
    if(ABBA != NULL){
        insere(ABBC, ABBA->chave);
        uniao(ABBA->esq, ABBC);
        uniao(ABBA->dir, ABBC);
    }
}
...
int main(){
    ...
    printf("\n\nArvoreAB: ");
    imprime(ABBC);
    printf("\n\nArvore de Busca Binaria A eh: ");
    if(verifABBE(ABBA,ABBA->chave) == 1)
        printf("[ INVALIDA ]\n");
    else
        printf("[ VALIDA ]\n");
    printf("\n\nArvore de Busca Binaria B eh: ");
    if(verifABBE(ABBB,ABBB->chave) == 1)
        printf("[ INVALIDA ]\n");
    else
        printf("[ VALIDA ]\n");
    printf("\n\nArvore de Busca Binaria C eh: ");
    if(verifABBE(ABBC,ABBC->chave) == 1)
        printf("[ INVALIDA ]\n");
    else
        printf("[ VALIDA ]\n");
    destruct(ABBA);
